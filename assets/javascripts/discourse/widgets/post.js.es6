...
createWidget('post-meta-data', {
  ...
  html(attrs) {
    ...
    if (createdAt) {
      result.push(h('div.text-muted.small',
        h('a.post-date', {
          attributes: {
            href: attrs.shareUrl,
...
createWidget('post-article', {
  ...
  html(attrs, state) {
    ...
    if (state.repliesAbove.length) {
      ...
      rows.push(h('div', h('section.embedded-posts.top.topic-body.offset2', replies)));
    }

    rows.push(h('div', [this.attach('post-avatar', attrs), this.attach('post-body', attrs)]));
    return rows;
  },
